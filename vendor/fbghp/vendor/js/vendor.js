...
function propFilter( props, specialEasing ) {
	...
	for ( index in props ) {
		...
		if ( hooks && "expand" in hooks ) {
			...
			// not quite jQuery.extend, this wont overwrite keys already present.
...
// jQuery.ui might exist from components with no dependencies, e.g., jQuery.ui.position
jQuery.ui = jQuery.ui || {};
	...
	jQuery.each( [ "Width", "Height" ], function( i, name ) {
		...
		function reduce( elem, size, border, margin ) {
			jQuery.each( side, function() {
				size -= parseFloat( jQuery.css( elem, "padding" + this ) ) || 0;
				if ( border ) {
					size -= parseFloat( jQuery.css( elem, "border" + this + "Width" ) ) || 0;
				}
				if ( margin ) {
					size -= parseFloat( jQuery.css( elem, "margin" + this ) ) || 0;
				}
			});
			return size;
		}
...
if ( $( "<a>" ).data( "a-b", "a" ).removeData( "a-b" ).data( "a-b" ) ) {
	jQuery.fn.removeData = (function( removeData ) {
		return function( key ) {
			if ( arguments.length ) {
				return removeData.call( this, jQuery.camelCase( key ) );
			} else {
...
// deprecated
jQuery.ui.ie = !!/msie [\w.]+/.exec( navigator.userAgent.toLowerCase() );
...
jQuery.extend( jQuery.ui, {
	// jQuery.ui.plugin is deprecated. Use jQuery.widget() extensions instead.
	plugin: {
...
jQuery.widget("ui.draggable", jQuery.ui.mouse, {
	version: "1.10.4",
	...
	_mouseStop: function(event) {
		...
		if ( this.options.helper === "original" && !jQuery.contains( this.element[ 0 ].ownerDocument, this.element[ 0 ] ) ) {
			return false;
		}
	...
	_mouseUp: function(event) {
		...
		if( jQuery.ui.ddmanager ) {
			jQuery.ui.ddmanager.dragStop(this, event);
		}

		return jQuery.ui.mouse.prototype._mouseUp.call(this, event);
	},
	...
	_createHelper: function(event) {
		...
		var o = this.options,
			helper = jQuery.isFunction(o.helper) ? $(o.helper.apply(this.element[0], [event])) : (o.helper === "clone" ? this.element.clone().removeAttr("id") : this.element);
	...
	_getParentOffset: function() {
		...
		if(this.cssPosition === "absolute" && this.scrollParent[0] !== document && jQuery.contains(this.scrollParent[0], this.offsetParent[0])) {
		...
		if((this.offsetParent[0] === document.body) ||
			(this.offsetParent[0].tagName && this.offsetParent[0].tagName.toLowerCase() === "html" && jQuery.ui.ie)) {
			po = { top: 0, left: 0 };
		}
	...
	_convertPositionTo: function(d, pos) {
		...
		var mod = d === "absolute" ? 1 : -1,
			scroll = this.cssPosition === "absolute" && !( this.scrollParent[ 0 ] !== document && jQuery.contains( this.scrollParent[ 0 ], this.offsetParent[ 0 ] ) ) ? this.offsetParent : this.scrollParent;
...
jQuery.ui.plugin.add("draggable", "connectToSortable", {
	start: function(event, ui) {
		...
		$(o.connectToSortable).each(function() {
			var sortable = jQuery.data(this, "ui-sortable");
...
jQuery.ui.plugin.add("draggable", "snap", {
	...
	drag: function(event, ui) {
		...
		for (i = inst.snapElements.length - 1; i >= 0; i--){
			...
			if ( x2 < l - d || x1 > r + d || y2 < t - d || y1 > b + d || !jQuery.contains( inst.snapElements[ i ].item.ownerDocument, inst.snapElements[ i ].item ) ) {
				if(inst.snapElements[i].snapping) {
					(inst.options.snap.release && inst.options.snap.release.call(inst.element, event, jQuery.extend(inst._uiHash(), { snapItem: inst.snapElements[i].item })));
				}
...
jQuery.ui.plugin.add("draggable", "stack", {
	start: function() {
		var min,
			...
			group = jQuery.makeArray($(o.stack)).sort(function(a,b) {
...
jQuery.widget("ui.droppable", {
	...
	_create: function() {
		...
		this.accept = jQuery.isFunction(accept) ? accept : function(d) {
			return d.is(accept);
		};
		...
		jQuery.ui.ddmanager.droppables[o.scope] = jQuery.ui.ddmanager.droppables[o.scope] || [];
		jQuery.ui.ddmanager.droppables[o.scope].push(this);
	...
	_destroy: function() {
		var i = 0,
			drop = jQuery.ui.ddmanager.droppables[this.options.scope];
	...
	_setOption: function(key, value) {
		...
		if(key === "accept") {
			this.accept = jQuery.isFunction(value) ? value : function(d) {
				return d.is(value);
			};
		}
	...
	_drop: function(event,custom) {
		...
		this.element.find(":data(ui-droppable)").not(".ui-draggable-dragging").each(function() {
			var inst = jQuery.data(this, "ui-droppable");
			if(
				inst.options.greedy &&
				!inst.options.disabled &&
				inst.options.scope === draggable.options.scope &&
				inst.accept.call(inst.element[0], (draggable.currentItem || draggable.element)) &&
				jQuery.ui.intersect(draggable, jQuery.extend(inst, { offset: inst.element.offset() }), inst.options.tolerance)
...
jQuery.ui.ddmanager = {
	...
	drop: function(draggable, event) {
		...
		jQuery.each((jQuery.ui.ddmanager.droppables[draggable.options.scope] || []).slice(), function() {
	...
	drag: function(draggable, event) {
		...
		jQuery.each(jQuery.ui.ddmanager.droppables[draggable.options.scope] || [], function() {
			...
			var parentInstance, scope, parent,
				intersects = jQuery.ui.intersect(draggable, this, this.options.tolerance),
			...
			if (this.options.greedy) {
				...
				parent = this.element.parents(":data(ui-droppable)").filter(function () {
					return jQuery.data(this, "ui-droppable").options.scope === scope;
				});

				if (parent.length) {
					parentInstance = jQuery.data(parent[0], "ui-droppable");
...
jQuery.widget("ui.sortable", jQuery.ui.mouse, {
	version: "1.10.4",
	...
	_mouseCapture: function(event, overrideHandle) {
		...
		$(event.target).parents().each(function() {
			if(jQuery.data(this, that.widgetName + "-item") === that) {
				currentItem = $(this);
				return false;
			}
		});
		if(jQuery.data(event.target, that.widgetName + "-item") === that) {
	...
	_mouseStop: function(event, noPropagation) {
		...
		//If we are using droppables, inform the manager about the drop
		if (jQuery.ui.ddmanager && !this.options.dropBehaviour) {
			jQuery.ui.ddmanager.drop(this, event);
		}
		...
		if(connectWith && connected) {
			for (i = connectWith.length - 1; i >= 0; i--){
				...
				for ( j = cur.length - 1; j >= 0; j--){
					inst = jQuery.data(cur[j], this.widgetFullName);
					if(inst && inst !== this && !inst.options.disabled) {
						queries.push([jQuery.isFunction(inst.options.items) ? inst.options.items.call(inst.element) : $(inst.options.items, inst.element).not(".ui-sortable-helper").not(".ui-sortable-placeholder"), inst]);
					}
				}
			}
		}

		queries.push([jQuery.isFunction(this.options.items) ? this.options.items.call(this.element, null, { options: this.options, item: this.currentItem }) : $(this.options.items, this.element).not(".ui-sortable-helper").not(".ui-sortable-placeholder"), this]);
	...
	_removeCurrentsFromItems: function() {
		...
		this.items = jQuery.grep(this.items, function (item) {
	...
	_refreshItems: function(event) {
		...
		var i, j, cur, inst, targetData, _queries, item, queriesLength,
			items = this.items,
			queries = [[jQuery.isFunction(this.options.items) ? this.options.items.call(this.element[0], event, { item: this.currentItem }) : $(this.options.items, this.element), this]],
			connectWith = this._connectWith();
		...
		if(connectWith && this.ready) { //Shouldn't be run the first time through due to massive slow-down
			for (i = connectWith.length - 1; i >= 0; i--){
				...
				for (j = cur.length - 1; j >= 0; j--){
					inst = jQuery.data(cur[j], this.widgetFullName);
					if(inst && inst !== this && !inst.options.disabled) {
						queries.push([jQuery.isFunction(inst.options.items) ? inst.options.items.call(inst.element[0], event, { item: this.currentItem }) : $(inst.options.items, inst.element), inst]);
	...
	_contactContainers: function(event) {
		...
		for (i = this.containers.length - 1; i >= 0; i--) {
			...
			if(jQuery.contains(this.currentItem[0], this.containers[i].element[0])) {
				continue;
			}
			...
			if(this._intersectsWith(this.containers[i].containerCache)) {
				...
				if(innermostContainer && jQuery.contains(this.containers[i].element[0], innermostContainer.element[0])) {
					continue;
				}
		...
		if(this.containers.length === 1) {
			...
			for (j = this.items.length - 1; j >= 0; j--) {
				if(!jQuery.contains(this.containers[innermostIndex].element[0], this.items[j].item[0])) {
					continue;
				}
	...
	_createHelper: function(event) {
		...
		var o = this.options,
			helper = jQuery.isFunction(o.helper) ? $(o.helper.apply(this.element[0], [event, this.currentItem])) : (o.helper === "clone" ? this.currentItem.clone() : this.currentItem);
	...
	_getParentOffset: function() {
		...
		// This needs to be actually done for all browsers, since pageX/pageY includes this information
		// with an ugly IE fix
		if( this.offsetParent[0] === document.body || (this.offsetParent[0].tagName && this.offsetParent[0].tagName.toLowerCase() === "html" && jQuery.ui.ie)) {
			po = { top: 0, left: 0 };
		}
	...
	_generatePosition: function(event) {
		...
		var top, left,
			...
			pageX = event.pageX,
			pageY = event.pageY,
			scroll = this.cssPosition === "absolute" && !(this.scrollParent[0] !== document && jQuery.contains(this.scrollParent[0], this.offsetParent[0])) ? this.offsetParent : this.scrollParent, scrollIsRootNode = (/(html|body)/i).test(scroll[0].tagName);
	...
	_trigger: function() {
		if (jQuery.Widget.prototype._trigger.apply(this, arguments) === false) {
			this.cancel();
		}
	},
...
(function(root, factory) {
  ...
  Backbone.ajax = function() {
    return Backbone.jQuery.ajax.apply(Backbone.$, arguments);
  };
